...
import com.lll.lookfor.utils.DateUtil;

/**
 * @author LIJUNJIE
 * 
 */
public class HooMsgBean implements Comparable<HooMsgBean>, Serializable {
	/**
	 * 
	 */
	...
	public String toString() {
		return "HooMsgBean [msgId=" + msgId + ", receiveId=" + receiveId
				...
				+ ", senderName=" + senderName + ", senderLogo=" + senderLogo
				+ ", content=" + content + ", contentType=" + contentType
				+ ", mediaName=" + mediaName + ", mediaId=" + mediaId
				+ ", mediaPoster=" + mediaPoster + ", sendTime=" + sendTime
				+ ", url=" + url + ", mediaStartTime=" + mediaStartTime
				+ ", mediaEndTime=" + mediaEndTime + ", channelId=" + channelId
				+ ", status=" + status + ", isChoice=" + isChoice + "]";
	}

	public String getInsertSql() {
		String escapeStr=(content!=null)?content.replace("'", "''"):"";//单引号字符转义
		String sql = MessageFormat.format(HooDBSql.HOO_SQL_MSG_INSERT,
				HooDBSql.HOO_DATABASE_MESSAGE, msgId + "", receiveId, senderId,
				msgType + "", subject, escapeStr, contentType + "", mediaName,
				mediaPoster, sendTime, senderName, senderLogo, status + "",
				mediaId + "", groupId, mediaStartTime, mediaEndTime, url,
				channelId);
		return sql;
	}
	...
	public int compareTo(HooMsgBean another) {
		if (another.getSendTime() != null && this.sendTime != null) {
			return DateUtil
					.compareMsgTime(this.sendTime, another.getSendTime());
		}
		return 0;
	}
}
